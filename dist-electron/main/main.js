...
let appSettings = {
    ...
    downloadFolder: electron_1.app.getPath('downloads') || path.join(process.env.USERPROFILE || '', 'Downloads'),
...
// Fetch Metadata
async function fetchMetadata(videoUrl, customCookiesPath) {
    return new Promise((resolve, reject) => {
        ...
        process.on('close', (code) => {
            ...
            try {
                ...
                const formats = (rawJson.formats || []).map((f) => {
                    let resolution = 'N/A';
                    if (f.resolution) {
                ...
                const uniqueFormats = formats.filter((f) => f.vcodec !== 'none' || f.acodec !== 'none');
                const metadata = {
...
const downloadQueue = [];
const activeProcesses = new Map(); // Map download id -> child process
...
function startDownloadProcess(item) {
    ...
    const dlProcess = (0, child_process_1.spawn)(cmd, args, { shell: true });
...
electron_1.ipcMain.handle('settings-save', (event, newSettings) => {
...
// Download IPCs
electron_1.ipcMain.handle('downloader-fetch-metadata', async (event, url, cookiesPath) => {
    return fetchMetadata(url, cookiesPath);
});
electron_1.ipcMain.handle('downloader-start', (event, url, options) => {
...
electron_1.ipcMain.handle('downloader-pause', (event, id) => {
    ...
    if (item && item.status === 'downloading') {
        ...
        if (proc && proc.pid) {
            ...
            (0, child_process_1.exec)(`taskkill /pid ${proc.pid} /T /F`, (err) => {
                if (err)
                    appendLog(`taskkill (pause) error: ${err.message}`);
            });
...
electron_1.ipcMain.handle('downloader-resume', (event, id) => {
    const item = downloadQueue.find((i) => i.id === id);
...
electron_1.ipcMain.handle('downloader-cancel', (event, id) => {
    const item = downloadQueue.find((i) => i.id === id);
    if (item) {
        const proc = activeProcesses.get(id);
        if (proc && proc.pid) {
            ...
            (0, child_process_1.exec)(`taskkill /pid ${proc.pid} /T /F`, (err) => {
                if (err)
                    appendLog(`taskkill (cancel) error: ${err.message}`);
            });
            activeProcesses.delete(id);
        }
...
electron_1.ipcMain.handle('downloader-retry', (event, id) => {
    const item = downloadQueue.find((i) => i.id === id);
    if (item) {
        item.status = 'queued';
        item.progress = 0;
...
electron_1.ipcMain.handle('downloader-clear-queue', () => {
    // Clear only finished/cancelled/failed items; keep active & queued ones
    const activeIds = new Set(activeProcesses.keys());
    const newQueue = downloadQueue.filter((item) => activeIds.has(item.id) || item.status === 'downloading' || item.status === 'queued' || item.status === 'paused');
...
// History / Shell IPCs
electron_1.ipcMain.handle('history-load', () => {
    return loadHistory();
});
...
electron_1.ipcMain.handle('open-folder', (event, dirPath) => {
    const pathToCheck = dirPath || appSettings.downloadFolder;
    if (fs.existsSync(pathToCheck)) {
        electron_1.shell.openPath(pathToCheck);
...
electron_1.ipcMain.handle('play-file', (event, filePath) => {
    ...
    if (fs.existsSync(absolutePath)) {
        electron_1.shell.openPath(absolutePath);
        ...
        if (match) {
            ...
            electron_1.shell.openPath(foundPath);
...
electron_1.ipcMain.handle('diagnostics-check', async () => {
